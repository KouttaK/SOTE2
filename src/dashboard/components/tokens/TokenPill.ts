...
const ICONS = {
  ...
  random: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="4" ry="4" stroke="currentColor" stroke-width="2"/><circle cx="8" cy="8" r="1.6" fill="currentColor"/><circle cx="16" cy="8" r="1.6" fill="currentColor"/><circle cx="12" cy="12" r="1.6" fill="currentColor"/><circle cx="8" cy="16" r="1.6" fill="currentColor"/><circle cx="16" cy="16" r="1.6" fill="currentColor"/></svg>`,
  flow_ref: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"><path d="M4 5h9a4 4 0 0 1 4 4v10" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M13 15l4 4 4-4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
};
...
export class TokenPill {
  static createHTML(token: Token, cursorNumber?: number): string {
    ...
    if (token.type === 'cursor' && cursorNumber !== undefined) {
      ...
      label = `${label} (${(token.config.options as unknown[]).length})`;
    } else if (token.type === 'flow_ref') {
